...
export async function exportEvidenceAnalysisExcel(
  ...
  appendObjectSheet(workbook, '全量标准化流水', transactions.map((transaction, index) => ({
    ...
    '原始识别金额': transaction.originalAmount ?? '',
    '原始识别余额': transaction.originalBalance ?? '',
    '原始识别方向': transaction.originalDirection || '',
    '自动修正说明': transaction.correctionReason || '',
    '识别复核状态': transaction.reviewStatus || '',
