...
echo "=== [1/4] 安装 Miniconda Python 3.10 环境 (官方国内镜像加速) ==="
if [ ! -d /opt/miniconda ]; then
  curl -sL https://mirrors.aliyun.com/anaconda/miniconda/Miniconda3-latest-Linux-x86_64.sh -o /tmp/miniconda.sh
  bash /tmp/miniconda.sh -b -p /opt/miniconda
  rm -f /tmp/miniconda.sh
fi

/opt/miniconda/bin/conda create -y -n ocr python=3.10 || true

echo "=== [2/4] 安装 PaddleOCR 与高精解析库 (阿里云镜像高速下载) ==="
/opt/miniconda/envs/ocr/bin/pip install --upgrade pip -i https://mirrors.aliyun.com/pypi/simple/
/opt/miniconda/envs/ocr/bin/pip install fastapi "uvicorn[standard]" python-multipart pypdfium2 pdfplumber opencv-python-headless pillow paddlepaddle paddleocr -i https://mirrors.aliyun.com/pypi/simple/
...
ExecStart=/opt/miniconda/envs/ocr/bin/uvicorn app:app --host 0.0.0.0 --port 8000
Restart=always
RestartSec=3
...
sleep 5
echo "=== 服务运行状态校验 ==="
systemctl status lawflow-ocr --no-pager || true
